...
{
    console.log("============ [3] ============");
    let num = Number(prompt("Enter the value of N (3rd) :-") ?? "0");
    let sum = 0;
    for (let i = 1; i <= num; i++) {
        sum += i;
    }
...
// Ans - 11
{
    ...
    if (totalAttend < 75)
        console.log(`Your Attendance is ${totalAttend}%\nYou are not eligible for exam.`);
    else
...
{
    console.log("============ [14] ============");
    function discountChecker(price) {
        if (price >= 1000) {
...
// Ans - 15
{
    ...
    if (attempt == 0)
        console.log("Incorrect Password (Refresh the Page to enter password again.");
    else if (attempt > 0)
